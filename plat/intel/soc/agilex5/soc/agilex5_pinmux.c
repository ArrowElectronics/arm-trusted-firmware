...
static void config_fpgaintf_mod(void)
{
	 ...
	 */
	fpgaintf_en_val = 0U;
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(NAND_USEFPGA)) & 0x01)
		fpgaintf_en_val |= BIT(4);
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(SDMMC_USEFPGA)) & 0x01)
		fpgaintf_en_val |= BIT(8);
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(SPIM0_USEFPGA)) & 0x01)
		fpgaintf_en_val |= BIT(16);
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(SPIM1_USEFPGA)) & 0x01)
	 ...
	 */
	fpgaintf_en_val = 0U;
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(EMAC0_USEFPGA)) & 0x01)
		fpgaintf_en_val |= BIT(0);
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(EMAC1_USEFPGA)) & 0x01)
		fpgaintf_en_val |= BIT(8);
	if (mmio_read_32(SOCFPGA_PINUMX_USEFPGA(EMAC2_USEFPGA)) & 0x01)
		fpgaintf_en_val |= BIT(16);
...
void config_pinmux(handoff *hoff_ptr)
{
	...
	for (i = 0; i < ARRAY_SIZE(hoff_ptr->pinmux_fpga_array); i += 2) {
